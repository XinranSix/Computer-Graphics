...
int main() {
    ...
    Model Zhang3 = Model("./assets/model/backpack/backpack.obj");
    _Models["zhang3"] = ModelInfo{&Zhang3, glm::vec3(-3, 0, 0), 0.0, 0.0, 0.0, false, "Zhang3"};
    Model Li4 = Model("./assets/model/backpack/backpack.obj");
    _Models["Li4"] = ModelInfo{&Li4, glm::vec3(3, 0, 0), 0.0, 0.0, 0.0, false, "Li4"};
//    Model Wang4 = Model("./assets/model/nanosuit/nanosuit.obj");
//    _Models["Wang4"] = ModelInfo{&Wang4, glm::vec3(0, 0, -3), 0.0, 0.0, 0.0, false, "Li4"};
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glStencilMask(0xFF);
        // model
        for (auto _modelsIter = _Models.begin();
...
void mouseClick_callback(GLFWwindow *window, int button, int action, int mods) {
    ...
    if ((action == GLFW_PRESS) && (button == GLFW_MOUSE_BUTTON_RIGHT)) {
        ...
        for (map<string, ModelInfo>::iterator _modelsIter = _Models.begin();
            ...
            if (_distance < 2.5) {
                cout << _modelsIter->first << "Model is selected..." << endl;
...
glm::vec3 worldPosFromViewPort(int winX, int winY) {
    ...
    glReadPixels(
            winX,
            (int) SCR_HEIGHT - winY, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &winZ);
