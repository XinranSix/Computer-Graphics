...
float lerp1(float a, float b, float f) { return a + f * (b - a); }
...
int main() {
    ...
    // -------------------------
    Shader shaderGeometryPass("shaders/ssao_geometry.vs",
                              "shaders/ssao_geometry.fs");
    ...
    for (unsigned int i = 0; i < 64; ++i) {
        ...
        scale = lerp1(0.1f, 1.0f, scale * scale);
        sample *= scale;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // 4. lighting pass: traditional deferred Blinn-Phong lighting with
