...
当有多个三角形需要光栅化是，我们需要正确处理三角形的前后关系，以保证前面的三角形会正确地遮挡后面的三角形。
...
深度缓存会记录每个像素最小的深度值，会生成（Depth buffer）和颜色缓存（Frame buffer）遍历所以三角形上的点，对于任意一个像素，我们记录下这个像素上深度最小的深度信息和颜色信息（假设深度是正数）。Z-Buffer 算法的描述如下：
