...
Radiant Energy：指的是电磁辐射的能量（The energy of electromagnetic radiation），用符号 $Q$ 表示，单位是焦耳（$J$）。

Radiant flux：指的是单位时间内的能量，用符号 $\Phi$ 表示，单位是瓦特（$W$），也可以用单位流明（$lm$）。
$$
\Phi = \frac{\mathrm{Q}}{\mathrm{d}t}
$$

### Radiant Intensity

Radiant Intensity 指的是光源在单位立体角上的能量。数学定义为：
$$
I\left(\omega \right) = \frac{\mathrm{d}\Phi}{\mathrm{d}\omega}
$$
